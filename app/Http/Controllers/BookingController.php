...
class BookingController extends Controller
{
    ...
    public function bookingStore(Request $request)
    {
        if (Auth::check()) {
            ...
            $user_booking = User_Booking::withTrashed()->count();
            // dd($user_booking);
            if ($user_booking == 0) {
                $booking_id = 'BTL0001';
            } else {
                $numRow = $user_booking + 1;

                if ($numRow < 10) {
                    $booking_id = 'BTL' . '000' . $numRow;
                } elseif ($numRow >= 10 && $numRow <= 99) {
                    $booking_id = 'BTL' . '00' . $numRow;
                } elseif ($numRow >= 100 && $numRow <= 999) {
                    $booking_id = 'BTL' . '0' . $numRow;
                } elseif ($numRow >= 1000 && $numRow <= 9999) {
                    $booking_id = 'BTL' . $numRow;
                }
            }
            ...
            foreach ($check_time_1 as $row) {
                ...
                if ($waktu_awal < $waktu_awal_db && $waktu_akhir > $waktu_awal_db) {
                    return redirect()->back()->with('message_fail', 'Jam sudah terbooking di jam ' . $waktu_awal_db . '!');
                }
            }
            ...
            if ($check_data == 1) {
                return redirect()->back()->with('message_fail', 'Jam sudah ada yang booking, dari jam ' . $check_time->jam_mulai . '-' . $check_time->jam_habis . '!');
            }
